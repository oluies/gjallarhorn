...
// smoke checks.
//
// Runtime: ~18-30s including add-friend handshake + dial + several
// convo rounds.
//
// Wires harnessConvoState.Logf on both clients to t.Logf so any
// future test failure prints alice's and bob's per-round Seal /
// Open / DeadDrop key material side-by-side for diagnosis. Logs
// only print on test failure (t.Logf behavior).
